...
客户端JavaScript不允许读取或写入文件。 这是出于安全原因而保留的。c file  fopen('','w+')
...
一套操作浏览器功能的API  window.open('')  window.screen.width

通过BOM可以操作浏览器窗口，比如：弹出框、控制浏览器跳转、获取分辨率等

### DOM - 文档对象模型 document.write() printf();
...
- 下面哪些变量名不合法。
  ...
  18age
  name
  $name【php变量的命名 $name='zang' var name='zang'】
  _sex
  &sex
  theworld  theWorld
  ```
...
- parseFloat()
  ...
  parseFloat会解析第一个. 遇到第二个.或者非数字结束    var a='33.3.' parseInt 33 paeseFloat 33.3
  如果解析的内容里只有整数，解析成整数
  ```
...
### 逻辑运算符(布尔运算符)
	&& 与 两个操作数同时为true，结果为true，否则都是false，前操作数如果为false ，则后操作数不验证
	|| 或 两个操作数有一个为true，结果为true，否则为false，前操作数如果为true ，则后操作数不验证
	!  非  取反
